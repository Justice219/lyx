...
local setFont = lyx.SetFont
local getTextSize = lyx.GetTextSize
local setTextPos = surface.SetTextPos
local setTextColor = surface.SetTextColor
local drawText = surface.DrawText

function lyx.DrawSimpleText(text, font, x, y, col, xAlign, yAlign)
...
function lyx.DrawText(text, font, x, y, col, xAlign, yAlign)
    local curX = x
    local curY = y
...
function lyx.DrawShadowText(text, font, x, y, col, xAlign, yAlign, depth, shadow)
    shadow = shadow or 50
...
local drawShadowText = lyx.DrawShadowText

function lyx.DrawDualText(title, subtitle, x, y, h)
    x = x or 0
    y = y or 0
    ...
    drawShadowText(title[1], title[2], x, y - sH * 0.5, title[3], title[4], 1, title[5], title[6])
    drawShadowText(subtitle[1], subtitle[2], x, y + tH * 0.5, subtitle[3], subtitle[4], 1, subtitle[5], subtitle[6])
end
...
function lyx.WrapText(text, width, font) --Edit of https://github.com/FPtje/DarkRP/blob/master/gamemode/modules/base/cl_util.lua#L21
...
function lyx.EllipsesText(text, width, font)
    local chachedName = text .. width .. font
