...
function PANEL:Init()
    self.TextEntry = vgui.Create("lyx.TextEntry", self)
    self.Message = vgui.Create("lyx.Label", self)
    ...
    self.TextEntry.OnChange = function(s)
        ...
        if valid then
            ...
            self.Message:SetTextColor(lyx.Colors.Positive)

            s.OverrideCol = lyx.Colors.Positive
        else
            self.Message:SetText(message or "")
            self.Message:SetTextColor(lyx.Colors.Negative)

            s.OverrideCol = lyx.Colors.Negative
        end
    end
end
...
vgui.Register("lyx.ValidatedTextEntry2", PANEL, "Panel")