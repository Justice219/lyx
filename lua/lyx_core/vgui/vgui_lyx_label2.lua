...
lyx.RegisterFont("UI.Label", "Open Sans SemiBold", 14)

function PANEL:Init()
    self:SetText("Label")
    self:SetFont("UI.Label")
    self:SetTextAlign(TEXT_ALIGN_LEFT)
    self:SetTextColor(lyx.Colors.SecondaryText)
end
...
function PANEL:Paint(w, h)
    ...
    if align == TEXT_ALIGN_CENTER then
        lyx.DrawText(text, self:GetFont(), w * 0.5, 0, self:GetTextColor(), TEXT_ALIGN_CENTER)
        return
...
vgui.Register("lyx.Label2", PANEL, "Panel")