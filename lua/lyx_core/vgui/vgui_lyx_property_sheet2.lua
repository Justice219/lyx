...
lyx.RegisterFont("UI.Tab", "Open Sans Bold", 16)

function PANEL:Init()
	self.BackgroundCol = lyx.OffsetColor(lyx.Colors.Background, -4)
	self.SelectedCol = lyx.Colors.Primary
	self.UnselectedTextCol = lyx.Colors.SecondaryText
	self.SelectedTextCol = lyx.Colors.PrimaryText
...
function PANEL:DoRightClick()
	...
	local tabs = vgui.Create("lyx.Menu", self)
...
function PANEL:Paint(w, h)
	...
	lyx.DrawRoundedBoxEx(lyx.Scale(6), 0, 0, w, h, self.Color, true, true)
...
function PANEL:Init()
	...
	self.BackgroundCol = lyx.OffsetColor(lyx.Colors.Background, 2)
end

function PANEL:AddSheet(label, panel, material, noStretchX, noStretchY, tooltip)
	if not IsValid(panel) then
		ErrorNoHalt("lyx.PropertySheet:AddSheet tried to add invalid panel!")
		debug.Trace()
		return
	end
	...
	sheet.Tab = vgui.Create("lyx.Tab", self)
...
function PANEL:Paint(w, h)
	...
	lyx.DrawRoundedBox(lyx.Scale(4), 0, offset, w, h - offset, self.BackgroundCol)
end

vgui.Register("lyx.PropertySheet2", PANEL, "Panel")