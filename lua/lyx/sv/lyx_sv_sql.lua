...
function lyx:SQLCreate(name, values)
    ...
    lyx:Log("Printing last SQL Error for debugging purposes, ")
    print(sql.LastError())
end
...
function lyx:SQLUpdateAll(name, row, value)
    lyx:Log("Updating all entries in DB table: " .. name)
...
function lyx:SQLLoad(name, method)
    ...
    if !val then
        lyx:Log("Could not load data from DB table: " .. name .. " with method: " .. method)    
        return false
    else
        return val
    end
