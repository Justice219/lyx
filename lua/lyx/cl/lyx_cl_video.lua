...
-- Variables
local medialib = include("lyx/thirdparty/medialib.lua")
lyx.vid = nil
lyx.mediaClip = nil
...
function lyx:VideoCheck(enable, url, tbl)
    if enable then
        if lyx.vid then
            ...
            lyx.mediaClip = nil
            lyx.vid = nil
            lyx:Log("Video stopped")
        end

        lyx:VideoCreate(url, tbl)
        lyx:Log("Video created")
    else
        if lyx.vid then
            lyx:HookRemove("HUDPaint", lyx.vid)
            lyx.mediaClip:stop()
