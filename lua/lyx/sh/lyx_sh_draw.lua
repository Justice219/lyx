...
function lyx:DrawCircle(pos,tbl)
    ...
    lyx:Log("Drawing circle at "..tostring(pos).." with radius "..tostring(radius).." and width "..tostring(width))
    return drawing
end
...
function lyx:DrawBox(pos, tbl)
    ...
    lyx:Log("Drawing box at "..tostring(pos).." with angle "..tostring(ang).." and min "..tostring(min).." and max "..tostring(max))
    return drawing
end
...
if CLIENT then
    ...
    concommand.Add("lyx_print_drawing", function(ply, cmd, args)
        for k,v in pairs(lyx.drawing) do
            lyx:Log(tostring(k).." "..tostring(v.type).." at "..tostring(v.pos).." with angle "..tostring(v.ang).." and min "..tostring(v.min).." and max "..tostring(v.max))
        end
    end)
    ...
    concommand.Add("lyx_clear_drawing", function(ply, cmd, args)
        for k,v in pairs(lyx.drawing) do
            ...
            lyx:Log("Removed drawing "..tostring(k))
        end
        lyx.drawing = {}
        lyx:Log("Cleared all drawings")
    end)
end