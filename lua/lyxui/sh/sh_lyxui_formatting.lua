--[[
	LYXUI Formatting Utilities
	Ported from PIXEL UI.

	Provides money and time formatting functions.
	LYXUI.FormatMoney() - Formats a number as currency with commas
	LYXUI.FormatTime()  - Formats seconds into a human-readable duration
]]
...
do
    ...
    --- Loads currency settings from the active gamemode config
    local function load()
    ...
    load()
    hook.Add("PostGamemodeLoaded", "LYXUI.GetMoneyFormatSettings", load)
    ...
    do
        ...
        -- @param val number The monetary value to format
        -- @return string Formatted currency string (e.g. "$1,234,567")
        function LYXUI.FormatMoney(val)
...
-- @return string Formatted string (e.g. "1w 2d 3h 4m 5s")
function LYXUI.FormatTime(time)
