--[[
	LYXUI Image Button Element
	Ported from PIXEL UI. Button that displays a URL-sourced image icon.
]]
...
function PANEL:Init()
    ...
    self:SetImageURL(LYXUI.ProgressImageURL)
...
function PANEL:Paint(w, h)
    ...
    if not self:IsEnabled() then
        LYXUI.DrawImage(imageOffset, imageOffset, imageSize, imageSize, self:GetImageURL(), self:GetDisabledColor())
        return
    end
    ...
    LYXUI.DrawImage(imageOffset, imageOffset, imageSize, imageSize, self:GetImageURL(), self.ImageCol)
end

vgui.Register("LYXUI.ImageButton", PANEL, "LYXUI.Button")
