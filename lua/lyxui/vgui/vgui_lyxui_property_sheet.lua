--[[
	LYXUI Property Sheet Element
	Ported from PIXEL UI. Tabbed panel container with animated tab switching.
]]
...
lyx.RegisterFont("LYXUI.Tab", "Open Sans Bold", 16)

function PANEL:Init()
    self.BackgroundCol = lyx.OffsetColor(LYXUI.Colors.Background, -4)
    self.SelectedCol = LYXUI.Colors.Primary
    self.UnselectedTextCol = LYXUI.Colors.SecondaryText
    self.SelectedTextCol = LYXUI.Colors.PrimaryText

    self.Color = lyx.CopyColor(self.BackgroundCol)
    self.TextColor = lyx.CopyColor(self.UnselectedTextCol)
end
...
function PANEL:Setup(text, propertySheet, panel)
    ...
    lyx.SetFont("LYXUI.Tab")
    self:SetWide(lyx.GetTextSize(text) + lyx.Scale(16))
end
...
function PANEL:DoRightClick()
    ...
    local tabs = vgui.Create("LYXUI.Menu", self)
...
function PANEL:Paint(w, h)
    self.Color = lyx.LerpColor(FrameTime() * 12, self.Color, (self:IsActive() or self:IsHovered()) and self.SelectedCol or self.BackgroundCol)
    self.TextColor = lyx.LerpColor(FrameTime() * 12, self.TextColor, (self:IsActive() or self:IsHovered()) and self.SelectedTextCol or self.UnselectedTextCol)

    LYXUI.DrawRoundedBoxEx(lyx.Scale(6), 0, 0, w, h, self.Color, true, true)
    lyx.DrawSimpleText(self:GetText(), "LYXUI.Tab", w * .5, h * .5, self.TextColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end

vgui.Register("LYXUI.Tab", PANEL, "LYXUI.Button")
...
function PANEL:Init()
    ...
    self.tabScroller:SetOverlap(lyx.Scale(5))
    self.tabScroller:Dock(TOP)
    self.tabScroller:DockMargin(lyx.Scale(3), 0, lyx.Scale(3), 0)

    self:SetFadeTime(0.1)
    self:SetPadding(lyx.Scale(8))

    self.animFade = Derma_Anim("Fade", self, self.CrossFade)

    self.Items = {}

    self.BackgroundCol = lyx.OffsetColor(LYXUI.Colors.Background, 2)
end

function PANEL:AddSheet(label, panel, material, noStretchX, noStretchY, tooltip)
    if not IsValid(panel) then
        ErrorNoHalt("LYXUI.PropertySheet:AddSheet tried to add invalid panel!")
        debug.Trace()
        return
    end
    ...
    sheet.Tab = vgui.Create("LYXUI.Tab", self)
    ...
    sheet.Panel:SetPos(self:GetPadding(), lyx.Scale(24) + self:GetPadding())
...
function PANEL:Paint(w, h)
    ...
    LYXUI.DrawRoundedBox(lyx.Scale(4), 0, offset, w, h - offset, self.BackgroundCol)
end

vgui.Register("LYXUI.PropertySheet", PANEL, "Panel")
