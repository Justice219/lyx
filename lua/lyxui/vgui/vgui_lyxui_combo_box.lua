--[[
	LYXUI Combo Box Element
	Ported from PIXEL UI. Dropdown selection box with sortable options.
]]
...
function PANEL:PerformLayout(w, h)
    ...
    self:SizeToText()
    self:SetWide(self:GetWide() + lyx.Scale(14))
end
...
function PANEL:ChooseOption(value, index)
    ...
    if not self:GetSizeToText() then return end
    self:SizeToText()
    self:SetWide(self:GetWide() + lyx.Scale(10))
end
...
function PANEL:OpenMenu(pControlOpener)
    ...
    CloseDermaMenus()
    self.Menu = vgui.Create("LYXUI.Menu", self)
    ...
    self.Menu:Open(x, y + lyx.Scale(6), false, self)
...
function PANEL:PaintOver(w, h)
    ...
    LYXUI.DrawImage(w - dropBtnSize - lyx.Scale(8), h / 2 - dropBtnSize / 2, dropBtnSize, dropBtnSize, "https://pixel-cdn.lythium.dev/i/5r7ovslav", LYXUI.Colors.PrimaryText)
end

vgui.Register("LYXUI.ComboBox", PANEL, "LYXUI.TextButton")
