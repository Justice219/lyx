--[[
	LYXUI Menu Element
	Ported from PIXEL UI. Dropdown/context menu with sub-menu support.
]]
...
function PANEL:Init()
    ...
    self:SetMinimumWidth(lyx.Scale(100))
...
function PANEL:AddOption(strText, funcFunction)
    local pnl = vgui.Create("LYXUI.MenuOption", self)
    pnl:SetMenu(self)
    pnl:SetText(strText)
...
function PANEL:AddCVar(strText, convar, on, off, funcFunction)
    local pnl = vgui.Create("LYXUI.MenuOptionCVar", self)
    pnl:SetMenu(self)
    pnl:SetText(strText)
    if funcFunction then pnl.DoClick = funcFunction end
...
function PANEL:AddSpacer(text, func)
    ...
    local spacerCol = lyx.OffsetColor(LYXUI.Colors.Background, 6)
    pnl.Paint = function(p, w, h)
        surface.SetDrawColor(spacerCol)
        surface.DrawRect(0, 0, w, h)
    end

    pnl:SetTall(lyx.Scale(3))
    self:AddPanel(pnl)
...
vgui.Register("LYXUI.Menu", PANEL, "LYXUI.ScrollPanel")
