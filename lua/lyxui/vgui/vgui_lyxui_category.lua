--[[
	LYXUI Category Element
	Ported from PIXEL UI. Collapsible category container with animated expand/collapse.
]]
...
lyx.RegisterFont("LYXUI.CategoryHeader", "Open Sans Bold", 19)

function PANEL:Init()
    self.ArrowRotation = 0

    self.BackgroundCol = lyx.OffsetColor(LYXUI.Colors.Background, 6)
end
...
function PANEL:Paint(w, h)
    ...
    lyx.DrawSimpleText(self.Title, "LYXUI.CategoryHeader", lyx.Scale(10), h / 2, LYXUI.Colors.PrimaryText, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)

    self.ArrowRotation = lerp(FrameTime() * 10, self.ArrowRotation, self:GetParent():GetExpanded() and 0 or 90)

    local arrowSize = h * .45
    LYXUI.DrawImageRotated(w - h * .3 - lyx.Scale(4), h / 2, arrowSize, arrowSize, self.ArrowRotation, "https://pixel-cdn.lythium.dev/i/5r7ovslav", LYXUI.Colors.PrimaryText)
end

vgui.Register("LYXUI.CategoryHeader", PANEL, "LYXUI.Button")
...
function PANEL:Init()
    self.Header = vgui.Create("LYXUI.CategoryHeader", self)

    self:SetTitle("LYXUI Category")
...
function PANEL:PerformLayout(w, h)
    ...
    self.Header:SetTall(lyx.Scale(26))
...
vgui.Register("LYXUI.Category", PANEL, "Panel")
