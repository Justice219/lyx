--[[
	LYXUI Menu Option Elements
	Ported from PIXEL UI. Menu option items including CVar-bound variant.
]]
local PANEL = {}
...
lyx.RegisterFont("LYXUI.MenuOption", "Open Sans SemiBold", 18)

function PANEL:Init()
    self:SetTextAlign(TEXT_ALIGN_LEFT)
    self:SetFont("LYXUI.MenuOption")
    self:SetChecked(false)

    self.NormalCol = LYXUI.Colors.Transparent
    self.HoverCol = LYXUI.Colors.Scroller
...
function PANEL:AddSubMenu()
    local subMenu = vgui.Create("LYXUI.Menu", self)
...
function PANEL:Paint(w, h)
    ...
    lyx.DrawSimpleText(self:GetText(), self:GetFont(), lyx.Scale(14), h / 2, LYXUI.Colors.PrimaryText, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)

    if not self.SubMenu then return end
    local dropBtnSize = lyx.Scale(8)
    LYXUI.DrawImage(w - dropBtnSize - lyx.Scale(6), h / 2 - dropBtnSize / 2, dropBtnSize, dropBtnSize, "https://pixel-cdn.lythium.dev/i/ce2kyfb88", LYXUI.Colors.PrimaryText)
end
...
vgui.Register("LYXUI.MenuOption", PANEL, "LYXUI.Button")
...
vgui.Register("LYXUI.MenuOptionCVar", PANEL, "LYXUI.MenuOption")
