--[[
	LYXUI String Request Popup
	Ported from PIXEL UI. Text input dialog with confirm/cancel buttons.
]]
...
function PANEL:LayoutContent(w, h)
    ...
    self.TextEntry:SetTall(lyx.Scale(32))
    self.TextEntry:Dock(TOP)
    self.TextEntry:DockMargin(0, 0, 0, lyx.Scale(10))
...
vgui.Register("LYXUI.StringRequest", PANEL, "LYXUI.Frame")

LYXUI.UI.Overrides.Derma_StringRequest = LYXUI.UI.Overrides.Derma_StringRequest or Derma_StringRequest

Derma_StringRequest = LYXUI.UI.CreateToggleableOverride(LYXUI.UI.Overrides.Derma_StringRequest, function(title, text, placeholderText, enterCallback, cancelCallback, buttonText, cancelText)
    cancelCallback = cancelCallback or function() end
    buttonText = buttonText or "OK"
    cancelText = cancelText or "Cancel"

    local msg = vgui.Create("LYXUI.StringRequest")
    msg:SetTitle(title)
    msg:SetText(text)
