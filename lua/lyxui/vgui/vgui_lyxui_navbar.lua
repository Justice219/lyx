--[[
	LYXUI Navbar Elements
	Ported from PIXEL UI. Horizontal navigation bar with animated selection indicator.
]]
...
lyx.RegisterFont("LYXUI.NavbarItem", "Open Sans SemiBold", 22)

function PANEL:Init()
    self:SetName("N/A")
    self:SetColor(LYXUI.Colors.Primary)

    self.NormalCol = LYXUI.Colors.PrimaryText
    self.HoverCol = LYXUI.Colors.SecondaryText

    self.TextCol = lyx.CopyColor(self.NormalCol)
end

function PANEL:GetItemSize()
    lyx.SetFont("LYXUI.NavbarItem")
...
function PANEL:Paint(w, h)
    ...
    lyx.DrawSimpleText(self:GetName(), "LYXUI.NavbarItem", w / 2, h / 2, self.TextCol, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end

vgui.Register("LYXUI.NavbarItem", PANEL, "LYXUI.Button")
...
function PANEL:Init()
    ...
    self.BackgroundCol = lyx.OffsetColor(LYXUI.Colors.Background, 10)
end

function PANEL:AddItem(id, name, doClick, order, color)
    local btn = vgui.Create("LYXUI.NavbarItem", self)

    btn:SetName(name:upper())
    btn:SetZPos(order or table.Count(self.Items) + 1)
    btn:SetColor((IsColor(color) and color) or LYXUI.Colors.Primary)
...
function PANEL:Paint(w, h)
    ...
    if not self.SelectedItem then
        ...
        self.SelectionColor = lyx.LerpColor(FrameTime() * 10, self.SelectionColor, LYXUI.Colors.Primary)
        return
    end
...
vgui.Register("LYXUI.Navbar", PANEL, "Panel")
