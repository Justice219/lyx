--[[
	LYXUI Color Picker Element
	Ported from PIXEL UI. HSL triangle color picker with hue wheel.
]]

local PANEL = {}

local gradientMat = Material("nil")
LYXUI.GetImage("https://pixel-cdn.lythium.dev/i/zxlflz5vp", function(mat)
    gradientMat = mat
end)

local colorWheelMat = Material("nil")
LYXUI.GetImage("https://pixel-cdn.lythium.dev/i/4lsnfph3b", function(mat)
    colorWheelMat = mat
end)

local pickerMat = Material("nil")
LYXUI.GetImage("https://pixel-cdn.lythium.dev/i/rhz6llj2", function(mat)
    pickerMat = mat
end)
...
function PANEL:UpdateColor()
    self.Color = lyx.HSLToColor(self.Hue, self.Saturation, self.Lightness)
...
function PANEL:GetHueColor()
    return lyx.HSLToColor(self.Hue, 1, 0.5)
end
...
function PANEL:Paint(w, h)
    ...
    local pickerVerts = {
        ...
        {
            x = cX + self.TriX * triangleRadius - lyx.Scale(5),
            y = cY + self.TriY * triangleRadius + lyx.Scale(5),
            u = 0,
            v = 1
        },
        {
...
vgui.Register("LYXUI.ColorPicker", PANEL, "Panel")
