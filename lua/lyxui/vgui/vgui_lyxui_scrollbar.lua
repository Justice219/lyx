--[[
	LYXUI Scrollbar Elements
	Ported from PIXEL UI. Scrollbar grip and scrollbar with smooth scrolling.
]]

local PANEL = {}

function PANEL:Init()
    self.NormalCol = LYXUI.Colors.Scroller
    self.HoverCol = lyx.OffsetColor(self.NormalCol, 15)

    self.Colour = lyx.CopyColor(self.NormalCol)
end
...
function PANEL:Paint(w, h)
    self.Colour = lyx.LerpColor(FrameTime() * 12, self.Colour,
        (self:IsHovered() or self:GetParent().Dragging) and self.HoverCol or self.NormalCol
    )

    LYXUI.DrawRoundedBox(w / 2, 0, 0, w, h, self.Colour)
end

vgui.Register("LYXUI.ScrollbarGrip", PANEL, "Panel")
...
function PANEL:Init()
    ...
    self.BackgroundCol = lyx.OffsetColor(LYXUI.Colors.Background, 5)

    self.Scrollbar = vgui.Create("LYXUI.ScrollbarGrip", self)
...
function PANEL:Paint(w, h)
    LYXUI.DrawRoundedBox(w / 2, 0, 0, w, h, self.BackgroundCol)
end

vgui.Register("LYXUI.Scrollbar", PANEL, "Panel")
